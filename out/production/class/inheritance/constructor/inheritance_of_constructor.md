...
2. 在派生类的构造方法中显式地调用直接基类的构造方法，如果调用无参构造方法，那么需要通过语句 ```super()```，如果调用有参构造方法，那么需要通过语句 ```super(参数列表)```. 并且调用直接基类的构造方法的语句必须是派生类的构造方法的第一个被执行的语句.
...
在类的构造方法中可以调用该类的其他重载的构造方法，调用的方法如下:

1. 在无参构造方法中，通过语句 ```this(参数列表)``` 调用该类自身的其他有参构造方法. 并且 ```this(参数列表)``` 语句必须是无参构造方法的第一个语句，因此这时不能在无参构造方法中使用 ```super``` 或 ```super(参数列表)``` 语句:
...
2. 在有参构造方法中，通过语句 ```this()``` 或 ```this(参数列表)``` 调用该类自身的无参构造方法或者其他的有参构造方法，并且 ```this()``` 或 ```this(参数列表)``` 语句也必须是构造方法的第一个语句，同理，这时不能在构造方法中使用 ```super()``` 和 ```super(参数列表)``` 语句:
...
3. 在任何一个构造方法中，都不能通过 ```this()``` 或 ```this(参数列表)``` 语句调用该构造方法自身. 因为在 Java 中，构造方法不能被递归地调用.